...
def analyze_innovation_growth(metrics: list, financial_line_items: list) -> dict:
    ...
    rd_expenses = [getattr(fi, 'research_and_development', None) for fi in financial_line_items if getattr(fi, 'research_and_development', None) is not None]
    revenues = [getattr(fi, 'revenue', None) for fi in financial_line_items if getattr(fi, 'revenue', None) is not None]
    ...
    fcf_vals = [getattr(fi, 'free_cash_flow', None) for fi in financial_line_items if getattr(fi, 'operating_margin', None) is not None]
